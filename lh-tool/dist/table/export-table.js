...
var export_table_exports = {};
__export(export_table_exports, {
  exportTable: () => exportTable,
  genTables: () => genTables,
  getModeTableNames: () => getModeTableNames
});
module.exports = __toCommonJS(export_table_exports);
...
async function exportTable(config) {
  try {
    ...
    if (!(0, import_fs_extra.existsSync)(config.exeFile)) {
      return {
        success: false,
        message: `\u6253\u8868\u5DE5\u5177\u4E0D\u5B58\u5728: ${config.exeFile}`
      };
    }
    if (!(0, import_fs_extra.existsSync)(config.dataDir)) {
      return {
        success: false,
    ...
    await (0, import_fs_extra.ensureDir)(config.tempDir);
    await (0, import_fs_extra.emptyDir)(config.tempDir);
    console.log("\u2705 \u4E34\u65F6\u76EE\u5F55\u5DF2\u6E05\u7A7A");
    console.log("\u5F00\u59CB\u6267\u884C\u6253\u8868...");
...
async function execTableTool(config) {
  const exePath = (0, import_path.dirname)(config.exeFile);
  const exeName = (0, import_path.basename)(config.exeFile);
  const args = [
    "-work_path",
    config.dataDir,
    "-code_path",
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = (0, import_child_process.spawn)(exeName, args, {
      stdio: "pipe",
      windowsHide: true
    });
    let stdout = "";
...
async function copyFiles(config) {
  const copiedFiles = [];
  if (!(0, import_fs_extra.existsSync)(config.tempDir)) {
    console.warn(`\u4E34\u65F6\u76EE\u5F55\u4E0D\u5B58\u5728: ${config.tempDir}`);
    return copiedFiles;
  }
  await (0, import_fs_extra.ensureDir)(config.exportDataDir);
  await (0, import_fs_extra.ensureDir)(config.codeDir);
  const files = (0, import_fs_extra.readdirSync)(config.tempDir);
  const copyTasks = [];
  for (const file of files) {
    const srcPath = (0, import_path.join)(config.tempDir, file);
    const stat = (0, import_fs_extra.statSync)(srcPath);
    if (!stat.isFile()) {
      continue;
    }
    ...
    let destPath;
    if (ext === ".json") {
      destPath = (0, import_path.join)(config.exportDataDir, file);
      copiedFiles.push(destPath);
      copyTasks.push(
        (0, import_fs_extra.copyFile)(srcPath, destPath).then(() => {
          console.log(`\u590D\u5236 JSON: ${file} -> ${destPath}`);
        })
      );
    } else if (ext === ".ts") {
      destPath = (0, import_path.join)(config.codeDir, file);
      copiedFiles.push(destPath);
      copyTasks.push(
        (0, import_fs_extra.copyFile)(srcPath, destPath).then(() => {
...
function getModeTableNames(dataDir, exportMode) {
  ...
  if (!(0, import_fs_extra.existsSync)(dataDir)) {
    console.error(`\u6570\u636E\u76EE\u5F55\u4E0D\u5B58\u5728: ${dataDir}`);
    return void 0;
  }
  const tableConfig = (0, import_path.join)(dataDir, "config.json");
  try {
    let tableNames;
    if (!(0, import_fs_extra.existsSync)(tableConfig)) {
      console.log(`\u8868\u683C\u914D\u7F6E\u6587\u4EF6\u4E0D\u5B58\u5728: ${tableConfig}`);
      return void 0;
    }
    const data = (0, import_fs_extra.readFileSync)(tableConfig, "utf-8");
    if (data) {
      tableNames = [];
    ...
    console.log(`\u627E\u5230 ${(tableNames == null ? void 0 : tableNames.length) || 0} \u4E2A\u8868\u683C`);
    return tableNames;
  } catch (err) {
...
function genTables(tableNames, outputPath) {
  ...
  try {
    ...
    (0, import_fs_extra.ensureDirSync)((0, import_path.dirname)(outputPath));
    (0, import_fs_extra.writeFileSync)(outputPath, tableTs, "utf-8");
