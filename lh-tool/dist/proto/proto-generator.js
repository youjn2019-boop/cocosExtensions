...
// source/proto/proto-generator.ts
var proto_generator_exports = {};
__export(proto_generator_exports, {
  ProtoGenerator: () => ProtoGenerator
});
module.exports = __toCommonJS(proto_generator_exports);
var import_fs_extra = __toESM(require_lib());
var import_path = require("path");
var ProtoGenerator = class {
  constructor() {
    this.protocols = [];
    this.protocolMap = /* @__PURE__ */ new Map();
    this.usedProtocolsSC = /* @__PURE__ */ new Set();
    // 跟踪已使用的协议SC
    this.usedProtocolsCS = /* @__PURE__ */ new Set();
  }
  // 跟踪已使用的协议CS
  /**
   * 读取并解析 JSON 数据
   */
  loadProtocolData(jsonPath) {
    try {
      const data = (0, import_fs_extra.readFileSync)(jsonPath, "utf8");
   ...
   * 解析参数类型
   */
  parseParameterType(usedProtocols, paramValue) {
...
`;
   ...
   * 生成独立的协议类
   */
  generateIndependentClasses(usedProtocols, indent) {
    let content = "";
    for (const protocol of this.protocols) {
      const { n: name, a: description, p: properties } = protocol;
...
`;
   ...
   */
  generateModuleClasses(usedProtocols, properties, indent) {
    let content = "";
    for (const [key, value] of Object.entries(properties)) {
      if (Array.isArray(value)) {
        const [type, description] = value;
...
`;
   ...
   * 生成类属性
   */
  generateClassProperties(usedProtocols, properties, indent) {
    let content = "";
    for (const [key, value] of Object.entries(properties)) {
...
`;
   ...
   * 生成 JS 模块
   */
  generateJSModule(properties, modulePrefix, indent) {
    let content = "";
...
`;
   ...
   * 生成 JS 类
   */
  generateJSClasses(properties, classPrefix, indent) {
    let content = "";
    const entries = Object.entries(properties);
    for (let i = 0; i < entries.length; i++) {
      const [key, value] = entries[i];
   ...
   */
  generate(inputPath, outputDir, dtsFileName = "proto.d.ts", jsFileName = "proto.js") {
    if (!(0, import_fs_extra.existsSync)(outputDir)) {
      (0, import_fs_extra.mkdirSync)(outputDir, { recursive: true });
    }
    this.loadProtocolData(inputPath);
    const dtsContent = this.generateTypeScriptDeclarations();
    const dtsPath = (0, import_path.join)(outputDir, dtsFileName);
    (0, import_fs_extra.writeFileSync)(dtsPath, dtsContent, "utf8");
    console.log(`\u751F\u6210 TypeScript \u58F0\u660E\u6587\u4EF6: ${dtsPath}`);
    const jsContent = this.generateJavaScriptImplementation();
    const jsPath = (0, import_path.join)(outputDir, jsFileName);
    (0, import_fs_extra.writeFileSync)(jsPath, jsContent, "utf8");
